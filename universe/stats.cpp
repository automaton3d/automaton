...
void drawConsole()
{
  // Calculate console area (bottom 2/3 of window, lifted upward with bottom margin)
  ...
  float consoleBottom = -1.0f + bottomMargin;
  ...
  int maxVisibleLines = (consoleHeightPixels - 20) / charHeight;
  int startIdx = 0;
  if (consoleLines.size() > (size_t)maxVisibleLines)
    startIdx = consoleLines.size() - maxVisibleLines;
  float lineHeight = (consoleTop - consoleBottom - 0.1f) / (maxVisibleLines > 0 ? maxVisibleLines : 1);
  float textY = consoleTop - 0.08f;
  for (int i = (int)consoleLines.size() - 1; i >= startIdx; --i)
  {
