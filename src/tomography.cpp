...
void renderTomoPlane() {
    ...
    // Mesma lógica usada no isVoxelVisible
//    float coord = (state.sliderPosition - 0.5f) * 0.5f;   // <--- CORRIGIDO
...
// centro do voxel (igual ao render)
float coord = (sliceIndex - HALF + 0.5f) * CELL_SPACING;
    ...
    if (tomoDirs[0].isSelected()) {  // XY plane (corte Z)
        vertices = {
    ...
    } else if (tomoDirs[1].isSelected()) {  // YZ plane (corte X)
        vertices = {
    ...
    } else if (tomoDirs[2].isSelected()) {  // ZX plane (corte Y)
        vertices = {
    ...
    glUseProgram(colorProgram3D);
    glUniformMatrix4fv(colorMvpLoc3D, 1, GL_FALSE, glm::value_ptr(mvp));
    glUniform3f(colorColorLoc3D, 0.4f, 0.4f, 0.4f);  // cinza escuro
