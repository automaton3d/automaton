...
int initializeCUDA()
{
    ...
    if (err != cudaSuccess) {
        std::cerr << "FATAL ERROR: cudaGetDeviceCount failed: " << cudaGetErrorString(err) << "\n";
        return 1;
    }
    if (deviceCount == 0) {
    ...
    cudaSetDevice(0);
    initGPU(automaton::EL, automaton::W_USED);

    automaton::setupHostBuffers();

    setSimulationParameters(automaton::EL, automaton::W_USED, 1);

    std::cout << "Attempting initGPULatticeState...\n";
    
    initGPULatticeState();
    //initializeSimulationForGPU(automaton::EL, automaton::W_USED, 1);//scenario=1

    std::cout << "initGPULatticeState succeeded.\n";

    //GPUEnabled = true;
    std::cout << "Running on GPU (CUDA)\n";

    return 0;

#else
    // Non-CUDA path (or emulation)
    GPUEnabled = false;  // Explicitly disable GPU mode
    std::cout << "Running on CPU (CUDA disabled)\n";
    return 0;
#endif
}
...
{
	// GPU initialization aqui
	if (initializeCUDA() != 0) {
        std::cerr << "CUDA (emulation) init failed - falling back to CPU\n";
        GPUEnabled = false;  // Force CPU mode if init fails
    }
...
int main()
{
    ...
    if (textRenderer) {
        ...
        std::cout << "Cleanup completed. Exiting successfully.\n";
