...
namespace automaton
{
  namespace polarization
  {
    ...
    // packed visited bits, EL^3/8 bytes per layer
    ...
    static inline void ensureSized()
    {
      if (g_axis.size() != W_USED || g_walk.size() != W_USED)
      {
        ...
        for (unsigned w = 0; w < W_USED; ++w)
          g_visit[w].assign(((size_t)EL * EL * EL + 7) / 8, 0);
      }
    }
    ...
    static bool diffusePass(unsigned w)
    {
      ...
      auto relax = [&](int x, int y, int z)
      {
        ...
        if (x < ELi - 1)  { n = stampAt(x + 1, y, z); if (n > best) best = n; }
        if (y > 0)        { n = stampAt(x, y - 1, z); if (n > best) best = n; }
        if (y < ELi - 1)  { n = stampAt(x, y + 1, z); if (n > best) best = n; }
        if (z > 0)        { n = stampAt(x, y, z - 1); if (n > best) best = n; }
        if (z < ELi - 1)  { n = stampAt(x, y, z + 1); if (n > best) best = n; }
      ...
      {
        for (int x = ELi - 1; x >= 0; --x)
        for (int y = ELi - 1; y >= 0; --y)
        for (int z = ELi - 1; z >= 0; --z)
          relax(x, y, z);
      }
