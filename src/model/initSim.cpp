...
void initGeneral()
{
    ...
    {
        // Centro específico desta camada
        ...
        for (unsigned x = 0; x < EL; ++x)
        {
            for (unsigned y = 0; y < EL; ++y)
            {
                for (unsigned z = 0; z < EL; ++z)
                {
                    ...
                    // Calcula distância em relação ao centro da camada
                    int dx = (int)x - (int)cx;
                    int dy = (int)y - (int)cy;
                    int dz = (int)z - (int)cz;
                    ...
                    if (r2 <= R2) {
                        ...
                        cell.d = RMAX;
                        cell.a = W_USED;  // Órfão fora da esfera
                    }
...
void initCenters(unsigned wDim)
{
    ...
    // Distribui os centros em um padrão esférico ao redor do centro global
    int R_center = EL / 4;  // Raio de distribuição dos centros
    
    for (unsigned w = 0; w < wDim; ++w)
    {
        // Ângulos para distribuir os centros uniformemente
