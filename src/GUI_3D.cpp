...
namespace framework {
  ...
  {
    const float GRID_SIZE = 0.5f / automaton::EL;
    const int CENTER_INT = automaton::EL / 2;
    std::vector<glm::vec3> pts;

    for (unsigned x=0;x<automaton::EL;x++)
      for (unsigned y=0;y<automaton::EL;y++)
        for (unsigned z=0;z<automaton::EL;z++) {
          int wx=(x+vis_dx+automaton::EL)%automaton::EL;
          int wy=(y+vis_dy+automaton::EL)%automaton::EL;
          int wz=(z+vis_dz+automaton::EL)%automaton::EL;
          if (getCell(lattice_curr,wx,wy,wz,layerList->getSelected()).pB) {
            float px,py,pz;
            if (currentMode==REPLAY) {
              auto& c = automaton::lcenters[layerList->getSelected()];
  ...
  {
	  const float CELL_SPACING = 0.5f / automaton::EL;  // Changed from 1.0f to 0.5f
      ...
      for (unsigned x=0;x<automaton::EL;x++)
        for (unsigned y=0;y<automaton::EL;y++)
          for (unsigned z=0;z<automaton::EL;z++) {
            uint32_t color = voxels[x*automaton::L2 + y*automaton::EL + z];
            if (color==0) continue;

            float px = (mod(x+vis_dx, automaton::EL) - CENTER_INT) * CELL_SPACING;
            float py = (mod(y+vis_dy, automaton::EL) - CENTER_INT) * CELL_SPACING;
            float pz = (mod(z+vis_dz, automaton::EL) - CENTER_INT) * CELL_SPACING;
  ...
  void renderCenters()
  {
    ...
    const float GRID_SIZE = 0.5f / automaton::EL;  // Changed from 1.0f / EL
    const int CENTER_INT = automaton::EL / 2;
    std::vector<glm::vec3> pts;
    ...
    for (unsigned w = 0; w < automaton::W_USED; w++)
    {
      ...
      automaton::Cell &cell = getCell(automaton::lattice_curr, cx, cy, cz, w);

      float px = ((int)(((int)cell.x[0] + vis_dx) % automaton::EL) - CENTER_INT) * GRID_SIZE;
      float py = ((int)(((int)cell.x[1] + vis_dy) % automaton::EL) - CENTER_INT) * GRID_SIZE;
      float pz = ((int)(((int)cell.x[2] + vis_dz) % automaton::EL) - CENTER_INT) * GRID_SIZE;
  ...
  {
    const float GRID_SIZE = 0.5f / automaton::EL;
    const float half = automaton::EL * GRID_SIZE / 2.0f;
    const float eps = -1e-4f;

    std::vector<glm::vec3> grid;
    for (unsigned i = 0; i <= automaton::EL; ++i)
    {
  ...
  void enhanceVoxel()
  {
    ...
    {
      const auto& center = automaton::lcenters[w];
      cx = ((center[0] + vis_dx) - automaton::EL / 2) * GRID_SIZE;
      cy = ((center[1] + vis_dy) - automaton::EL / 2) * GRID_SIZE;
      cz = ((center[2] + vis_dz) - automaton::EL / 2) * GRID_SIZE;
    }
    else
    {
    	automaton::Cell &cell = getCell(automaton::lattice_curr, automaton::CENTER, automaton::CENTER, automaton::CENTER, w);
      cx = ((cell.x[0] + vis_dx) - automaton::EL / 2) * GRID_SIZE;
      cy = ((cell.x[1] + vis_dy) - automaton::EL / 2) * GRID_SIZE;
      cz = ((cell.x[2] + vis_dz) - automaton::EL / 2) * GRID_SIZE;
    }
